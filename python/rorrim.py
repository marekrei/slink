...
class Asset:
    ...
    def get_base(self):
        base_tag = re.compile('<\s*base[^\>]*href\s*=\s*[\""\']?([^\""\'\s>]*).*?>', re.IGNORECASE|re.DOTALL|re.MULTILINE)
    ...
    def get_page_title(self):
        title_tag = re.compile('<\s*title[^>]*>(?P<context>.*?)<\/title>', re.IGNORECASE|re.DOTALL|re.MULTILINE)
    ...
    def get_links(self):
        ...
        for relation, regex in self.link_types.iteritems():
            pattern = re.compile(regex, re.IGNORECASE|re.DOTALL|re.MULTILINE)
